...
The following function generates tables that compare the parameters and goodness of fit metrics of the optimal fit, where the values of $\mu$, $\sigma$, $k_1$ and $k_2$ are variables that can be fitters, with the a formalized or simplified fit where the values of $\mu$, $\sigma$, $k_1$ and $k_2$ are set to the values mentioned above. The following code chunk generates the comparison tables. The cells in red to the optimal model fit and the cells in blue correspond to the formalized model fit. 
